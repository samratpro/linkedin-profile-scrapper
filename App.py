...
start_page_labe = CTkLabel(page_frame, text="Start")
start_page_labe.grid(row=4, column=0, padx=10, pady=3)
start_page = CTkEntry(page_frame, width=35, border_width=1, fg_color=('black', 'white'), text_color=('white', 'black'))
...
end_page_labe = CTkLabel(page_frame, text="End")
end_page_labe.grid(row=4, column=1, padx=10, pady=3)
end_page = CTkEntry(page_frame, width=35, border_width=1, fg_color=('black', 'white'), text_color=('white', 'black'))
...
Output_Folder = CTkEntry(page_frame, width=80, border_width=1, fg_color=('black', 'white'), text_color=('white', 'black'))
...
worker = CTkEntry(page_frame, width=70, border_width=1, fg_color=('black', 'white'), text_color=('white', 'black'))
...
scrapermode_labe = CTkLabel(page_frame, text="Mode")
scrapermode_labe.grid(row=4, column=4, padx=10, pady=3)
scrapermode = CTkComboBox(page_frame, width=80, values=["Quick", "Details"], state='readonly', fg_color=('black', 'white'), text_color=('white', 'black'))
scrapermode.grid(row=5, column=4, padx=10, pady=10)
scrapermode.set("Quick")

Browser_Status_labe = CTkLabel(page_frame, text="Browser Status")
Browser_Status_labe.grid(row=4, column=5, padx=10, pady=3)
Browser_Status = CTkComboBox(page_frame, width=80, values=["hide", "show"], state='readonly', fg_color=('black', 'white'), text_color=('white', 'black'))
Browser_Status.grid(row=5, column=5, padx=10, pady=10)
Browser_Status.set("show")

sleep_labe = CTkLabel(page_frame, text="Sleep")
sleep_labe.grid(row=4, column=6, padx=10, pady=3)
sleep = CTkComboBox(page_frame, width=20, values=["1",'2',"3","4","5"], state='readonly', fg_color=('black', 'white'), text_color=('white', 'black'))
sleep.grid(row=5, column=6, padx=10, pady=10)
sleep.set("1")
...
def operation_start_thread():
    ...
    threads = worker.get()
    delay = sleep.get()
    mode = scrapermode.get()
    print(browser_status)
    operation_close_event.clear()
    scrapper_loop(get_url, get_start_page, get_end_page, get_Output_Folder,browser_status,threads,log,mode, start, delay, close_event=operation_close_event)
